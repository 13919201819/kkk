      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.div
            ...
            transition={{ type: 'spring', stiffness: 260, damping: 20 }}>
            <motion.button
              onClick={() => { setIsOpen(true); resetInactivity(); }}
              className="relative rounded-full flex items-center justify-center overflow-hidden touch-manipulation shadow-lg"
              style={{
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            {/* Modal */}
            <motion.div
              className="relative flex flex-col bg-slate-900 overflow-hidden will-change-transform w-full"
              style={{
                // Mobile: slides up from bottom, takes 92dvh with rounded top corners
                // Tablet+: centered card with max dimensions and fully rounded corners
                height: 'min(92dvh, 700px)',
                maxWidth: 'min(100%, 56rem)',
                borderRadius: 'clamp(1rem, 3vw, 1.5rem) clamp(1rem, 3vw, 1.5rem) 0 0',
              ...
              // Override border-radius on sm+ screens to be fully rounded
              >
              {/* Fully rounded on larger screens via a wrapper override */}
              <style>{`
                @media (min-width: 640px) {
                  .chat-modal { border-radius: 1.5rem !important; }
                }
              `}</style>
              ...
              {/* Header */}
              <div className="shrink-0 relative z-10 flex items-center justify-between px-4 sm:px-6 md:px-8 py-3 sm:py-4 border-b border-white/10 bg-white/5 backdrop-blur-xl">
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                  <div className="shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-inner">
                    <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-white" strokeWidth={2.5} />
                  </div>
                  <div className="min-w-0">
                    <h2 className="text-sm sm:text-base md:text-xl font-bold text-white leading-tight truncate">{t('chatbot.title')}</h2>
                    <p className="text-[10px] sm:text-xs text-gray-400 truncate">{t('chatbot.subtitle')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-2">
                  {isSpeaking && (
                    <motion.button
                      ...
                      className="p-1.5 sm:p-2 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-colors touch-manipulation"
                      whileTap={{ scale: 0.9 }}>
                  ...
                  <motion.button
                    onClick={() => { stopVoiceLoop(); stopSpeaking(); setIsOpen(false); }}
                    className="p-1.5 sm:p-2 rounded-full bg-white/10 border border-white/10 text-gray-300 hover:bg-white/20 transition-colors touch-manipulation"
                    whileTap={{ scale: 0.9 }}>
              ...
              {/* Messages Area */}
              <div className="flex-1 overflow-y-auto overscroll-contain px-3 sm:px-5 md:px-8 py-3 sm:py-4 space-y-3 sm:space-y-4 scroll-smooth">
                {messages.map(msg => (
                  <motion.div
                    key={msg.id}
                    ...
                    <div className={`flex flex-col gap-2 ${msg.type === 'user' ? 'max-w-[88%] items-end' : 'max-w-[94%] sm:max-w-[85%] items-start'}`}>
                      <div className={`flex items-start gap-2 ${msg.type === 'user' ? 'flex-row-reverse' : 'flex-row'} w-full`}>
                        {/* Avatar */}
                        <div className={`shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center ${msg.type === 'user' ? 'bg-gradient-to-br from-blue-500 to-purple-600' : 'bg-gradient-to-br from-gray-700 to-gray-600'}`}>
                          {msg.type === 'user'
                            ? <span className="text-white text-[10px] sm:text-xs font-bold">Y</span>
                            : <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" strokeWidth={2.5} />}
                        </div>
                        {/* Bubble */}
                        <div
                          className={`px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-sm leading-relaxed break-words ${msg.type === 'user' ? 'bg-gradient-to-br from-blue-600 to-purple-600 text-white shadow-md' : 'bg-white/10 border border-white/10 text-gray-100'}`}
                          style={{ wordBreak: 'break-word', overflowWrap: 'break-word' }}
                          dangerouslySetInnerHTML={{ __html: msg.type === 'bot' ? msg.text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') : msg.text }} />
                      </div>

                      {msg.type === 'bot' && (
                        <div className="w-full pl-9 sm:pl-10 space-y-3">

                          {/* Solutions Grid */}
                          {msg.showSolutions && (
                            <div className="mt-1">
                              <p className="text-[10px] sm:text-xs font-semibold text-gray-400 mb-2">Core AI Solutions:</p>
                              <div className="grid grid-cols-1 xs:grid-cols-2 gap-1.5 sm:gap-2">
                                {solutions.map((sol: Solution, i: number) => (
                                  <motion.button
                                    key={`sol-${i}`}
                                    onClick={() => window.open(sol.url, '_blank')}
                                    className="px-2.5 sm:px-3 py-2 sm:py-2.5 rounded-lg text-white text-xs sm:text-sm bg-purple-900/40 border border-purple-500/40 hover:bg-purple-800/50 active:bg-purple-800/60 transition-colors text-left truncate touch-manipulation"
                                    whileTap={{ scale: 0.98 }}>
                                    {sol.text}
                                  </motion.button>
                                ))}
                                <motion.button
                                  ...
                                  className="col-span-full px-3 py-2 rounded-lg text-white text-xs sm:text-sm bg-blue-600/20 border border-blue-500/30 flex items-center justify-center gap-1.5 sm:gap-2 hover:bg-blue-600/30 active:bg-blue-600/40 transition-colors touch-manipulation"
                                  whileTap={{ scale: 0.98 }}>
                                  <ArrowRight className="w-3 h-3" /> Go to Full Solutions Page
                                </motion.button>
                              </div>
                          ...
                          {msg.showIndustries && (
                            <div className="mt-1">
                              <p className="text-[10px] sm:text-xs font-semibold text-gray-400 mb-1">Domains We Serve:</p>
                              <p className="text-xs sm:text-sm text-gray-300 mb-3 leading-relaxed">
                                {industries.map((i: Industry) => i.text).join(' • ')}
                              </p>
                              <div className="flex flex-wrap gap-2">
                                <motion.button
                                  onClick={() => handleActionRedirect('domains')}
                                  className="px-3 py-1.5 sm:py-2 rounded-lg text-white text-xs sm:text-sm bg-green-600/20 border border-green-500/30 hover:bg-green-600/30 active:bg-green-600/40 transition-colors touch-manipulation"
                                  whileTap={{ scale: 0.95 }}>
                                  <ArrowRight className="w-3 h-3 inline mr-1" /> {t('chatbot.buttons.viewTechnologies')}
                                </motion.button>
                                <motion.button
                                  onClick={() => handleRedirectConfirmation('domains', false)}
                                  className="px-3 py-1.5 sm:py-2 rounded-lg text-white text-xs sm:text-sm bg-gray-600/20 border border-gray-500/30 hover:bg-gray-600/30 active:bg-gray-600/40 transition-colors touch-manipulation"
                                  whileTap={{ scale: 0.95 }}>
                                  {t('chatbot.buttons.notNow')}
                                </motion.button>
                              </div>
                          ...
                          {msg.showContactButtons && (
                            <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-1">
                              <motion.button
                                onClick={() => location.href = 'tel:+917622817924'}
                                className="flex items-center px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-white text-xs sm:text-sm bg-green-500/10 border border-green-500/30 hover:bg-green-500/20 active:bg-green-500/30 transition-colors touch-manipulation"
                                whileTap={{ scale: 0.95 }}>
                                <Phone className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 shrink-0" /> {t('chatbot.buttons.callUs')}
                              </motion.button>
                              <motion.button
                                onClick={() => location.href = 'mailto:info@clumoss.com'}
                                className="flex items-center px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-white text-xs sm:text-sm bg-blue-500/10 border border-blue-500/30 hover:bg-blue-500/20 active:bg-blue-500/30 transition-colors touch-manipulation"
                                whileTap={{ scale: 0.95 }}>
                                <Mail className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 shrink-0" /> {t('chatbot.buttons.emailUs')}
                              </motion.button>
                              <motion.button
                                onClick={() => handleActionRedirect('contact')}
                                className="flex items-center px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-white text-xs sm:text-sm bg-purple-500/10 border border-purple-500/30 hover:bg-purple-500/20 active:bg-purple-500/30 transition-colors touch-manipulation"
                                whileTap={{ scale: 0.95 }}>
                                <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 shrink-0" /> View Contact Page
                              </motion.button>
                            </div>
                          )}
                          ...
                          {msg.showScheduleButton && (
                            <motion.button
                              onClick={() => handleActionRedirect('schedule')}
                              className="mt-1 flex items-center justify-center px-4 py-2 rounded-lg text-white text-sm font-medium bg-gradient-to-r from-green-600 to-teal-600 border border-white/10 hover:shadow-lg active:opacity-90 transition-all touch-manipulation w-full sm:w-auto"
                              whileTap={{ scale: 0.95 }}>
                              <Calendar className="w-4 h-4 mr-2 shrink-0" /> {t('chatbot.buttons.bookNow')}
                            </motion.button>
                          )}
                          ...
                          {msg.showRedirectConfirmation && (
                            <div className="mt-1 p-3 rounded-xl bg-white/5 border border-white/10">
                              <p className="text-xs sm:text-sm font-medium text-gray-200 mb-3">
                                {t('chatbot.confirmRedirect', { page: msg.showRedirectConfirmation.toUpperCase() }) || `Confirm redirection to the ${msg.showRedirectConfirmation.toUpperCase()} page?`}
                              </p>
                              <div className="flex gap-2">
                                <motion.button 
                                  onClick={() => handleRedirectConfirmation(msg.showRedirectConfirmation!, true)}
                                  className="flex-1 px-2.5 sm:px-3 py-2 rounded-lg text-white text-xs sm:text-sm bg-green-600/20 border border-green-500/30 hover:bg-green-600/30 active:bg-green-600/40 touch-manipulation"
                                  whileTap={{ scale: 0.95 }}>
                                  {t('chatbot.buttons.yesRedirect')}
                                </motion.button>
                                <motion.button 
                                  onClick={() => handleRedirectConfirmation(msg.showRedirectConfirmation!, false)}
                                  className="flex-1 px-2.5 sm:px-3 py-2 rounded-lg text-white text-xs sm:text-sm bg-red-600/20 border border-red-500/30 hover:bg-red-600/30 active:bg-red-600/40 touch-manipulation"
                                  whileTap={{ scale: 0.95 }}>
                                  {t('chatbot.buttons.notNow')}
                                </motion.button>
                              </div>
                          ...
                          {msg.followUps && msg.followUps.length > 0 && !msg.showRedirectConfirmation && (
                            <div className="mt-1">
                              <p className="text-[10px] uppercase tracking-wider text-gray-500 font-bold mb-1.5 pl-1">Related Topics</p>
                              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                                {msg.followUps.map((s: FollowUp, i: number) => ( 
                                  <motion.button
                                    key={`fup-${i}`}
                                    onClick={() => handleFollowUp(s)}
                                    className="px-2.5 sm:px-3 py-1.5 rounded-full text-gray-200 text-xs border border-white/10 bg-white/5 hover:bg-white/10 active:bg-white/15 transition-colors touch-manipulation"
                                    whileTap={{ scale: 0.95 }}>
                                    {s.text} 
                                  </motion.button>
                                ))}
                ...
                {isTyping && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-start">
                    <div className="flex items-center gap-2 pl-1">
                      <div className="shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-gray-700 to-gray-600 flex items-center justify-center">
                        <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" strokeWidth={2.5} />
                      </div>
                      <div className="px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-2">
                        <div className="flex gap-1">
                          {[0, 0.2, 0.4].map(d => (
                            <motion.div
                              key={d}
              ...
              {/* Input Area */}
              <div
                className="shrink-0 px-3 sm:px-5 md:px-6 pt-3 pb-3 border-t border-white/10 bg-white/5 backdrop-blur-xl"
                style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom, 0.75rem))' }}>
                <div className="flex items-center gap-1.5 sm:gap-2 p-1.5 sm:p-2 rounded-xl bg-white/5 border border-white/10 focus-within:border-blue-500/50 transition-colors">
                  <input
                    ref={inputRef}
                    type="text"
                    value={message}
                    ...
                    onKeyPress={e => e.key === 'Enter' && !e.shiftKey && handleSend(message, false)}
                    placeholder={t('chatbot.placeholder')} 
                    className="flex-1 min-w-0 bg-transparent text-white placeholder-gray-500 outline-none text-sm px-1.5 sm:px-2"
                    disabled={isTyping}
                    autoComplete="off"
                    autoCorrect="off"
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <motion.button
                      ...
                      className={`p-2 rounded-lg transition-all touch-manipulation ${isListening ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : 'text-gray-400 hover:text-white hover:bg-white/10 active:bg-white/20'}`}
                      whileTap={{ scale: 0.9 }}
                      disabled={isTyping}
                    ...
                    <motion.button
                      onClick={isTyping ? () => abortControllerRef.current?.abort() : () => handleSend(message, false)}
                      className={`p-2 rounded-lg transition-all touch-manipulation ${message.trim() ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20 active:bg-blue-700' : 'text-gray-400 hover:text-white hover:bg-white/10 active:bg-white/20'} disabled:opacity-50`}
                      whileTap={{ scale: 0.9 }}
                      disabled={!message.trim() && !isTyping}
                      aria-label="Send">
                      {isTyping ? <X className="w-4 h-4 sm:w-5 sm:h-5" /> : <Send className="w-4 h-4 sm:w-5 sm:h-5" />}
                    </motion.button>
                  </div>
                </div>
                ...
                {(isSpeaking || isListening) && (
                  <motion.div
                    ...
                    className="flex items-center justify-center gap-1.5 mt-2 text-xs font-medium text-blue-400">
                    {isSpeaking
